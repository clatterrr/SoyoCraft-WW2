...
public class Apply : MonoBehaviour
{
    ...
    Vector3 ChangeCamera(Bounds aabb, bool CameraBack)
    {
        ...
        int rrr = Random.Range(0, 4);
        t = new Vector3(c.x + e.x * 2, c.y + e.y * 2 + 4f, c.z + e.z * 3 - 20f);
        if (CameraBack)
        {
           t = new Vector3(c.x + e.x * 2, c.y + e.y * 2 + 4f, c.z + e.z * 3 + 20f);
        }
        else
        {
            switch (rrr) {
                case 0:
                    {
                        t = new Vector3(c.x + e.x * 2, c.y + e.y + 4f, c.z + e.z - 20f);
                        break;
                    }
                case 1:
                    {
                        t = new Vector3(c.x + e.x * 1.5f, c.y + e.y + 6f, c.z + e.z - 18f);
                        break;
                    }
                case 2:
                    {
                        t = new Vector3(c.x + e.x * 2.5f, c.y + e.y + 4f, c.z - 10.0f);
                        break;
                    }
                default: break;
            }
        }
        Debug.Log(" Camera pos = " + t);
        return t;
    }
    ...
    struct Model
    {
        ...
        public void SetOriginTexture()
        {
            ...
            Texture2D tmpTexture = colorTexture;
            ...
            for (int k = 2; k <= 5; k++)
            {
                for (int i = 0; i < texture_range[k].x; i++)
                {
                    ...
                    int px = (int)(texture_start[k].x + i);
                    int py = tmpTexture.height - (int)(texture_start[k].y + they);
                    tmpTexture.SetPixel(px, py, colorTexture.GetPixel(px, py));
                }
            }
            ...
            ApplayMaterial(left, texture_start[2], texture_range[2], tmpTexture);

            ApplayMaterial(right, texture_start[3], texture_range[3], tmpTexture);

            ApplayMaterial(back, texture_start[4], texture_range[4], tmpTexture);

            ApplayMaterial(front, texture_start[5], texture_range[5], tmpTexture);
        }
        public void SetOriginScale()
        {
            ...
            tbrx = (pos[1].x - topbottom.x) * 0.5f * (total_int_len - cur_int_len) + pos[2].x;
            tbry = (pos[1].y - topbottom.y) * 0.5f * (total_int_len - cur_int_len) + pos[2].y;
            Vector3 leftpos = new Vector3(tbrx, tbry, pos[2].z);
            tbrx = (pos[1].x - topbottom.x) * 0.5f * (total_int_len - cur_int_len) + pos[3].x;
            tbry = (pos[1].y - topbottom.y) * 0.5f * (total_int_len - cur_int_len) + pos[3].y;
            Vector3 rightpos = new Vector3(tbrx, tbry, pos[3].z);
    ...
    void FixedUpdate()
    {
        ...
        for(int i = 0; i < groups.Count; i++)
        {
            Debug.Log(" i = " + i + " name = " + groups[i].name + " count = " + groups[i].name);
            for (int j = 0; j < groups[i].children.Count; j++)
            {
                Model tmp = parts[part_index];
        ...
        if (tmp2.frame >= tmp2.duration * 4)
        {
            ...
            Debug.Log("camear pos = " + Camera.main.transform.position);
        }
