package com.example.examplemod.entity.custom.Garden;
...
public class PoolCleanerEntity extends ThePlantEntity implements IAnimatable {

    private static final EntityDataAccessor<Boolean> ATTACKING =
            SynchedEntityData.defineId(PoolCleanerEntity.class, EntityDataSerializers.BOOLEAN);

    private static final EntityDataAccessor<Integer> STYLE =
            SynchedEntityData.defineId(PoolCleanerEntity.class, EntityDataSerializers.INT);
    private AnimationFactory factory = new AnimationFactory(this);

    public PoolCleanerEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        ...
        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.pool_cleaner.idle", true));
    ...
    public void tick() {
        ...
        if(this.launch_state){
            ...
            if(this.level.getBlockState(bp).getBlock() == Blocks.WATER && this.pool_float == false){
                this.setDeltaMovement(0,-0.3, 0.0f);
                this.pool_float = true;
            }else{
