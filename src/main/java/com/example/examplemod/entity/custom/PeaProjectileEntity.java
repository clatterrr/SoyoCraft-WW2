...
public class PeaProjectileEntity extends Projectile {
	private static final EntityDataAccessor<Boolean> HIT =
			SynchedEntityData.defineId(PeaProjectileEntity.class, EntityDataSerializers.BOOLEAN);
	private int counter = 0;

	public PeaProjectileEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
		super(pEntityType, pLevel);
	}

	public PeaProjectileEntity(Level pLevel, Player player) {
		super(ModEntityTypes.PEA_PROJECTILE.get(), pLevel);
		setOwner(player);
		BlockPos blockpos = player.blockPosition();
		double d0 = (double)blockpos.getX();
		double d1 = (double)blockpos.getY() - 0.2D;
		double d2 = (double)blockpos.getZ();
		this.moveTo(d0, d1, d2, this.getYRot(), this.getXRot());
	}

	public PeaProjectileEntity(Level pLevel, Entity entity) {
		super(ModEntityTypes.PEA_PROJECTILE.get(), pLevel);
		setOwner(entity);
		double d0 = entity.position().x ;
		double d1 = entity.position().y - 0.2D;
	...
	public void tick() {
		...
		double scale = 1.0D;
		//vec3 = new Vec3(vec3.x * scale, vec3.y * scale, vec3.z * scale);
		...
		double d0 = this.getX() + vec3.x * scale;
		double d1 = this.getY() + vec3.y * scale;
		double d2 = this.getZ() + vec3.z * scale;
		this.updateRotation();
