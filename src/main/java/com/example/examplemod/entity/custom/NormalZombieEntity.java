...
public class NormalZombieEntity extends Monster implements IAnimatable {
    ...
    public class StickGoal extends Goal {
        private final NormalZombieEntity zombie;

        public StickGoal() {
            zombie = NormalZombieEntity.this;
        }

        private int cool_down = 0;
        @Override
        public boolean canUse() {
            List<FlagZombieEntity> flag_zombie = this.zombie.getLevel().getEntitiesOfClass(FlagZombieEntity.class, this.zombie.getBoundingBox().inflate(8));
            return !flag_zombie.isEmpty();
        }

        public void tick() {
            List<FlagZombieEntity> flag_zombies = this.zombie.getLevel().getEntitiesOfClass(FlagZombieEntity.class, this.zombie.getBoundingBox().inflate(8));
            if(!flag_zombies.isEmpty()){
                FlagZombieEntity flag = flag_zombies.get(0);
                if(flag.getTarget() != null) {
                    this.zombie.setTarget(flag.getTarget());
                }
            }
        }
    }
