...
public class GrassProjectileEntity extends Projectile {
	private static final EntityDataAccessor<Boolean> HIT =
			SynchedEntityData.defineId(GrassProjectileEntity.class, EntityDataSerializers.BOOLEAN);
	private int counter = 0;

	private double ground = 0;

	public GrassProjectileEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
		super(pEntityType, pLevel);
	}

	public GrassProjectileEntity(Level pLevel, Player player) {
		super(ModEntityTypes.GRASS_PROJECTILE.get(), pLevel);
		setOwner(player);
	...
	public GrassProjectileEntity(Level pLevel, Entity entity) {
		super(ModEntityTypes.GRASS_PROJECTILE.get(), pLevel);
		setOwner(entity);
		double d0 = entity.position().x ;
		double d1 = entity.position().y + 2f;
		double d2 = entity.position().z ;
		this.ground = entity.blockPosition().getY();
		this.moveTo(d0, d1, d2, this.getYRot(), this.getXRot());
	}
	...
	public void tick() {
        ...
        */
		...
		if (this.getLevel().getBlockStates(this.getBoundingBox()).noneMatch(BlockBehaviour.BlockStateBase::isAir)) {
			...
			this.setDeltaMovement(new Vec3(vec3.x * 0.99f, vec3.y * 0.99f - 0.1f, vec3.z * 0.99f));
	...
	protected void onHit(HitResult hitResult) {
        ...
        */
		...
		if(hitResult.getType() == HitResult.Type.ENTITY && hitResult instanceof EntityHitResult entityHitResult) {
			...
			if(bp == Blocks.DIRT || bp == Blocks.GRASS || bp == Blocks.ICE || bp == Blocks.FROSTED_ICE || bp == Blocks.SNOW || bp == Blocks.SAND){
				...
				this.getLevel().explode(this, this.getX(), this.getY(), this.getZ(), 1.5F, Explosion.BlockInteraction.DESTROY);
			}
			this.entityData.set(HIT, true);
			counter = this.tickCount + 5;
		}
