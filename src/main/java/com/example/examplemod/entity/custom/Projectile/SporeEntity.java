...
public class SporeEntity extends Monster implements IAnimatable {

    private static final EntityDataAccessor<Boolean> ATTACKING =
            SynchedEntityData.defineId(NormalZombieEntity.class, EntityDataSerializers.BOOLEAN);
    private AnimationFactory factory = new AnimationFactory(this);

    public SporeEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
        ...
        if(!zombies.isEmpty()) {
            for (int i = 0; i < zombies.size(); i++) {
                ...
                if(z.getOnPos().getX() == this.getOnPos().getX() && z.getOnPos().getZ() == this.getOnPos().getZ()){
                    z.hurt(DamageSource.GENERIC, 0.5f);
